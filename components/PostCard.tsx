    ...
    <View style={[styles.container, hasShadow && shadowStyle]}>
      <View style={styles.header}>
        ...
        <View style={styles.userInfo}>
            <Avatar uri={post.user.image} size={hp(5.6)} rounded={theme.radius.md}/>
            <View style={{gap:2}}>
        ...
        <TouchableOpacity onPress={openPostDetails}>
            <Icon name="threeDotsHorizontal" size={hp(3.5)} strokeWidth={3} color="black" />
        </TouchableOpacity>
      </View>
      {/* Post Body & media */}
      <View style={styles.content}>
        <View style={styles.postBody}>
            {
                post?.body && <RenderHtml contentWidth={wp(100)} source={{html:post?.body}} tagsStyles={tagStyles}/>
            }
        </View>
        <View>
        {/* post imag */}
            {
                post?.file && post?.file?.includes("postImage") && (
                    <Image source={getSupabaseImageSrc(post?.file)} transition={100} style={styles.postmedia} contentFit="cover" />
                )
                }
        </View>
        {/* post v */}
        {
            post?.file && post?.file?.includes("postVideo") && (
                ...
                <TouchableOpacity style={styles.muteButton} onPress={() => setIsMuted(!isMuted)}>
                    <Icon name={isMuted ? "mute" : "unmute"} size={hp(3)} color="white" />
                </TouchableOpacity>
                </View>
            )
        }
